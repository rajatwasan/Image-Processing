...
Brighten(double factor)
{
  ...
  for (int i = 0; i < npixels; ++i)
  {
    pixels[i] *= factor; // Multiplying each pixel by factor
    pixels[i].Clamp(); // 
    /* code */
  }
...
ChangeContrast(double factor)
{
  ...
  {
    /* code */
    lum_avg += pixels[i].Luminance();
  }

  lum_avg /= npixels;

  R2Pixel greypixel(lum_avg, lum_avg, lum_avg, 1.0);
...
Composite(const R2Image& top, int operation)
{
  ...
  // Formula Used- blend(i,j) = (1-t) src(i,j) + t dst(i,j) (0 $ t $ 1) 
  // C = alpha of A * A + (1- alpha of A) * alpha of B * B
  // alpha =  alpha of A + (1-  alpha of A)  * alpha of B
